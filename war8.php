<!-- second call ----->
	...
	<div class="col-md-2 col-sm-4 col-xs-6  text-center" style="padding:20px;">
		<?php  if ($caller['user_username8'] == NULL){
				if ($caller['user_username7'] !== $current_user || $user['user_title'] >= 3){	
 ...
 <a href="callit.php?e=<?php echo $i;?>&a=8" class="btn btn-success"  style="margin-top:80px;">Call it</a>
...
<div class="modal fade bs-example-modal-sm" id="myModal_call_8_enemy_<?php echo $i;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
 ...
 <?php if ($score['plan'] == 'userfiles/screenshoots/noplan.jpg') {?>
                          <?php if ($caller['user_username8'] == $current_user){?>
 ...
 <!--   popup -->
                   <?php if ($score['plan'] == 'userfiles/screenshoots/noplan.jpg'){?>
                    ...
                    <?php } }?>          
    <?php }?>            
                    <?php if ($caller['user_username8'] == $current_user  || $user['user_title'] >= 4){?>
                        ...
                        <input type="hidden" name="war_warid" value="<?php echo $current_war;?>"/>
                        <input type="hidden" name="war_size" value="<?php echo $war_size;?>"/>
                            <input type="hidden" name="user_call" value="user_username8"/>
                            <input type="hidden" name="user_callit" value="<?php echo $caller['user_username8'];?>"/>
                            <input type="hidden" name="current_username" value="<?php echo $current_user;?>"/>  
                        <button class="btn btn-danger btn-xs" data-style="zoom-in" type="submit"  id="SubmitButton" value="Upload" style="margin-top:10;" onclick="return confirm('Are you sure you want to delete the call on Enemy <?php echo $i; ?> by <?php echo $caller['user_username8'];?>?')"/>Delete</button>
                        </form>
                    <?php }?>
