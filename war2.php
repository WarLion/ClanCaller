 ...
 <a href="callit.php?e=<?php echo $i;?>&a=2" class="btn btn-success">Call it</a>
        ...
        <?php } } }else{?>
            <ul class="list-unstyled">
                    ...
                    <li><p  class="user_title" style="text-align: center; font-size:15; color:#FFF;"><?php echo $caller['user_username2'];?></p></li>
         <?php    
        ...
        while($score = mysqli_fetch_array($result_score)){ 
          ?>  
                    <li><a href="#myModal_call_2_enemy_<?php echo $i;?>" data-toggle="modal"><img src="imagenes/th/<?php if($score['score'] == NULL){echo 'none';}else{ echo $score['score'];}?>.png" width="80" /></a></li>
   <!-- timer -->
           <?php 
		    if ($score['score'] == NULL){
		   $log_user = $caller['user_username2'];
...
<div class="modal fade bs-example-modal-sm" id="myModal_call_2_enemy_<?php echo $i;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
 ...
 <!--   popup -->    
                   <?php if ($score['plan'] == 'userfiles/screenshoots/noplan.jpg'){?>
                    ...
                    <?php } }?>   
    <?php }?>               
                    <?php if ($caller['user_username2'] == $current_user || $user['user_title'] >= 4){?>
                    	<?PhP if($caller['user_username3'] == NULL || $user['user_title'] >= 4){?>
                        ...
                        <input type="hidden" name="war_warid" value="<?php echo $current_war;?>"/>
                        <input type="hidden" name="war_size" value="<?php echo $war_size;?>"/>
                            <input type="hidden" name="user_call" value="user_username2"/>
                            <input type="hidden" name="user_callit" value="<?php echo $caller['user_username2'];?>"/>
                            <input type="hidden" name="current_username" value="<?php echo $current_user;?>"/>  
                        <button class="btn btn-danger btn-xs" data-style="zoom-in" type="submit"  id="SubmitButton" value="Upload" style="margin-top:10;" onclick="return confirm('Are you sure you want to delete the call on Enemy <?php echo $i; ?> by <?php echo $caller['user_username2'];?>?')"/>Delete</button>
                        </form>
                        <?php }?>
                    <?php }?>
            </ul> 
